...
// `alt` has to be a static export, so it cannot follow the language; the card
// itself does.
export const alt = "RSA TEAM";
...
export default async function Image({
  ...
  params: Promise<{ lang: string; seasonId: string; matchId: string }>;
}) {
  const { lang: rawLang, seasonId, matchId } = await params;
  // An image route cannot read the root parameter the pages use, so the
  // language comes from its own params.
  ...
  const long = matchDateLong(match.date, lang);
  const dateLine = `${long.charAt(0).toUpperCase()}${long.slice(1)}${
    match.kickoff ? ` · ${t.common.kickoffAt(match.kickoff)}` : ""
  }`;
    ...
    (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
          ...
          >
            {t.content.competitions[match.competition] ?? match.competition} · {season.label}
          </div>
        </div>
        ...
        >
          <div style={{ display: "flex" }}>
            {match.status === "postponed" ? t.common.postponed : dateLine}
          </div>
          <div
            style={{
              display: "flex",
