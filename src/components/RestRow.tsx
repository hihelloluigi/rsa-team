...
export default async function RestRow({ round }: { round: number }) {
  const { t } = await getI18n();
  const LINE = t.matches.restLine;
  return (
    <div className="border-b border-white/10 py-4 text-muted">
      ...
      <div className="hidden sm:flex items-center gap-4">
        <div className="w-24 shrink-0 text-xs uppercase tracking-widest">{t.common.round(round)}</div>
